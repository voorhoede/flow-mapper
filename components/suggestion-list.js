    ...
    <ul>
      {suggestions.map((suggestion) => {
        if (suggestion.subject.__typename === 'ProcessRecord') {
          return (
            <li key={suggestion.subject.id} className="border-b hover:bg-gray-50">
              <Link href={`/suggestion/${router.query.canvasId}/${suggestion.subject.phase.slug}/process/${suggestion.subject.id}`}>
        ...
        if (suggestion.subject.__typename === 'SystemElementRecord') {
          return (
            <li key={suggestion.subject.id} className="border-b hover:bg-gray-50">
              <Link href={`/suggestion/${router.query.canvasId}/${suggestion.subject.phase.slug}/element/${suggestion.subject.id}`}>
                <a className="flex items-center p-2 pr-0">
                  <img
